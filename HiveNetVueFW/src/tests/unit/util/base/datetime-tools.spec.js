...
test('datetime-tools test date-string convert', () => {
  let fmt = 'yyyy/MM/dd hh:mm:ss';
  const now = datetimeTools.now();
  console.debug('now', now);
  let strFmt = datetimeTools.formatToStr(now, fmt);
  console.debug('strFmt', strFmt, fmt);
  let dateFmt = datetimeTools.strToDate(strFmt, fmt);
  console.debug('dateFmt', dateFmt, fmt);
  let newStrFmt = datetimeTools.formatToStr(dateFmt, fmt);
  expect(newStrFmt).toEqual(strFmt);

  fmt = 'yy/M/d h:m:s';
  strFmt = datetimeTools.formatToStr(now, 'yy/M/d h:m:s');
  console.debug('strFmt', strFmt, fmt);
  dateFmt = datetimeTools.strToDate(strFmt, fmt);
  console.debug('dateFmt', dateFmt, fmt);
  newStrFmt = datetimeTools.formatToStr(dateFmt, fmt);
  expect(newStrFmt).toEqual(strFmt);
});
  ...
  'data': [
    {
      ...
      'children': [
        ...
        },
        {
          ...
          'children': [{ 'name': 'Operlog', 'path': 'operlog', 'hidden': false, 'component': 'monitor/operlog/index', 'meta': { 'title': '操作日志', 'icon': 'form', 'noCache': false, 'link': null }}, { 'name': 'Logininfor', 'path': 'logininfor', 'hidden': false, 'component': 'monitor/logininfor/index', 'meta': { 'title': '登录日志', 'icon': 'logininfor', 'noCache': false, 'link': null }}]
        }]
    },
